...
(function (global, oDoc, factory) {
  ...
  function validateType(file, fileType) {
    ...
    const suffix = fileName.substr(fileName.indexOf('.') + 1);
    let i = 0;
  ...
  function validateFile(_this, source, type) {
    ...
    const validateSizeResult = validateSize(_this, file);
    if (!validateSizeResult) {
      _this.inptEle.value = '';
      return false;
    }

    const validateTypeResult = validateType(file, type);
  ...
  function ValidateFileUpload(opt) {
    ...
    this.fileType = fileType || null; // 允许上传到文件类型，数组['jpg', 'jpeg', 'png', 'gif', 'bmp', 'docx', 'xls', 'pptx', 'txt', 'mp4', 'mp3']
    this.maxSize = maxSize || 100; // 默认允许上传最大2M的文件
  ...
  ValidateFileUpload.prototype.init = function (source) {
    return !!validateFile(this, source, this.fileType);
  };

  // 实例化类，并返回校验结果
  function validateFileUpload(opt) {
    const validateObj = new ValidateFileUpload(opt);
    const validateRes = validateObj.init(validateObj.inptEle);
